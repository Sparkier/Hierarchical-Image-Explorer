...
export function generateScale(
  ...
  return (v: number) => {
    ...
    const extentAbsMax = Math.max(Math.abs(extent[0]), Math.abs(extent[1])) * 2;
...
export class LinearScale {
  ...
  constructor(
    ...
    this.extentAbsMax =
      Math.max(Math.abs(this.extent[0]), Math.abs(this.extent[1])) * 2;
  }
  ...
  public scale(v: number) {
    return (
      (v / this.extentAbsMax + 0.5) * (this.domain - 2 * this.padding) +
      this.padding
    );
  }
  ...
  public invert(w: number) {
    return (
      (-1 * (this.extentAbsMax * (this.domain - 2 * w))) /
      (2 * (this.domain - 2 * this.padding))
    );
  }
}
